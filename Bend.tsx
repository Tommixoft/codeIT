// Canvas UI: Bend — scroll-on-a-cube-face effect.
// Adapted for dc-runtime x-import: no ESM imports (React comes from scope),
// exports via module.exports. On browsers without the experimental
// HTML-in-canvas API the children render directly in normal document flow,
// so the page keeps native window scrolling.
...
interface BendElements {
  source: HTMLCanvasElement;
  content: HTMLElement;
  output: HTMLCanvasElement;
}

interface BendInstance {
...
interface BendProps extends BendOptions {
  children?: any;
  className?: string;
  style?: any;
}
...
function Bend({ children, className, style, ...options }: BendProps) {
  ...
  if (!native) {
    // No HTML-in-canvas support: render children in normal document flow so
    // the page keeps native window scrolling.
    return (
...
module.exports = { Bend, createBend, supportsHtmlInCanvas };
if (typeof window !== "undefined") (window as any).Bend = Bend;
